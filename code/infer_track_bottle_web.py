...
#             print (dummy_coco_dataset.classes[1])
        ...
        if 1 not in classes and not in_hand_event:
            ...
            if gap_count >= args.gap_threshold:
                ...
                bottle_indexs = [i for i, j in enumerate(classes) if j == 40]
                for i in bottle_indexs:
                    mask = masks[:, :, i]
                    S_prime.append(mask)
                    
                
                WhichIsTaken, bottle_list = WhichIsTaken_fun(S, S_prime, bottle_list)
                S = {}
                S_prime = []
                whichObject_strEndFrame[WhichIsTaken] = [started_frame, ended_frame]
            ...
            #Ordering object from left to right
            bottle_indexs = [i for i, j in enumerate(classes) if j == 40]
...
def WhichIsTaken_fun(S, S_prime, bottle_list):
    Objects_minXor = {}
    for object_name, object_mask in S.iteritems():
        result_xor = []
        for object_prime_mask in S_prime:
